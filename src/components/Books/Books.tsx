...
const Books = () => {
  ...
  const [toggleBtn, setToggleBtn] = useState<{ [key: string]: boolean }>({});
  ...
  const authors = [
    ...new Set(data?.Result.map((option) => option.author.name)),
  ];
  const categories = [
    ...new Set(data?.Result.map((option) => option.category.name)),
  ];
  const features = [...new Set(data?.Result.map((option) => option.tag.name))];
  ...
  const handleToggleBtn = (choice: string): void => {
    setToggleBtn((prevToggles) => ({
      ...prevToggles,
      [choice]: !prevToggles[choice], // Toggle the specific choice
    }));
    handleToggle(choice);
    setOption(choice);
